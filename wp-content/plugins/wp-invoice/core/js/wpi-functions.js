...
function wpi_load_email_notification() {
  ...
  if (selected > 0) {
    ...
    jQuery.getJSON(ajaxurl, {action: 'wpi_get_notification_email', template_id: template_id_val, wpi_invoiceid: invoice_id},
      function(response){
...
function wpi_save_postboxes() {
  ...
  hidden = jQuery('.postbox').filter(':hidden').map(function() {return this.id;}).get().join(',');
  jQuery.post(ajaxurl, {
...
function wpi_select_payment_method(method, force, init) {
  ...
  if(jQuery("#" + method + ".wpi_billing_section_show").is(":checked")) {method_checked = true} else {method_checked = false;}
  if(method_checked) {
